...
Symbol::Symbol(std::string _name, std::string _type, std::string _kind) {
...
SymbolTable::SymbolTable(std::string _scope, SymbolTable* _parent)
    : scope(_scope), parent(_parent), lastAddedFunction(nullptr) {
}
...
Value ASTNode::evaluate(SymbolTable* currentScope) {
    ...
    case NODE_ASSIGN: {
        ...
        if (symbol->type != val.type) {
            std::cerr << "[EROARE SEMANTICA] Nu poti atribui " + val.type + " la " + symbol->type << "\n";
            exit(1);
        }

        symbol->value = val; //  SymbolTable
        return val;
    }

    case NODE_PRINT: {
        Value val = left->evaluate(currentScope); // evaluate left tree
    ...
    case NODE_OP: {
        ...
        if (l.type != r.type) {
            std::cerr << "[EROARE SEMANTICA] Tipuri incompatibile pentru " + op << "\n";
            exit(1);
        }
    ...
    case NODE_OTHER: {
        ...
        if (!function) { std::cerr << "Functie inexistenta!\n"; exit(1); }
        if (function->paramTypes.size() != arguments.size()) { std::cerr << "Eroare argumente!\n"; exit(1); }
